...
####################################################################################
# Find GPU with least used memory
# modified version of
# https://discuss.pytorch.org/t/it-there-anyway-to-let-program-select-free-gpu-automatically/17560
# modified b/c it seems the "Free" option is gone with the current cuda driver (or at least, I can't access it in my current ubuntu build)
def get_freer_gpu():
...
# custom implementation of sampling from Wishart distribution
...
class PriorLinear(nn.Module):
    ...
    def forward(self, nbatch, n, n_tilde=1):
        ...
        # for now, generating A~Bern(0.5)
        a = self.bern5.sample([nbatch,n,1]).to(device)
        ...
        # nbatch x s output
        if s==1:
            u = self.noise_sampler.sample([nbatch,self.num_a])
            beta = self.rn(u)
            # transform beta so that it falls in +/- self.radius and flip its sign at random
            beta = self.radius * self.tanh(beta) * (2*self.bern5.sample([nbatch,self.num_a,s]).to(device)-1)
        else:
            u = self.noise_sampler.sample([nbatch,self.num_a,s])
            beta = self.rn(u).squeeze(-1)
            # scale to surface of unit ball and then scale to have radius Unif(0,self.radius)
            beta = self.radius * (2*torch.rand(nbatch,self.num_a,s,device=device)-1) * self.softmax(beta)

        # append zeros if s<wdim
        if s<wdim:
            # nbatch x (wdim + 1) output
            beta = torch.cat((beta,torch.zeros(nbatch,self.num_a,wdim-s,device=device)),dim=2)
        
        if self.permute:
            # randomly permute indices (relevant if T not invariant to permutations of predictors)
            beta = shuffle_dim3(beta)

        # w_all is nbatch x (n+n_tilde) x wdim
        # beta is nbatch x self.num_a x wdim
        out = torch.matmul(dup_along_newdim(w_all,1,self.num_a),beta.unsqueeze(-1))
        
        # generate y
        # CAUTION: hard-coded for the case where self.num_a=2 and a takes values 0 and 1
        y = (1-a) * out[:,0,range(n),:] + a * out[:,1,range(n),:] + self.E.sample([nbatch,n])
        
        return w_all[:,range(-n_tilde,0),:], w_all[:,range(n),:], a, y, out[:,1,:,:] - out[:,0,:,:]
    
# ####################################################################################
# # create Prior neural network class
# # Linear regression

# class PriorMars(nn.Module):
#     # If permute is set to true, then Pi will permute the beta outputs
#     def __init__(self, hidden_size, num_hidden, wdim, s, M=10, permute=True, n_fixed = 5):
#         super(PriorMars, self).__init__()
...
#         # self.rn = ReLU_net(udim, num_hidden, hidden_size, 3*n_fixed, leaky=True)
#         self.rn = equivar_net(udim, num_hidden, hidden_size, 3*n_fixed, [-2], activation="leaky_relu", pool="mean")
#         # source of noise
...
#             w_obs = shuffle_dim3(w_obs)
        
#         rn_out = self.rn(self.noise_sampler.sample([nbatch,s]))
#         locs = dup_along_newdim(rn_out[:,:,range(n_fixed)],-3,n_obs)
#         slopes = rn_out[:,:,range(n_fixed,2*n_fixed)]
#         sgns = self.tanh(rn_out[:,:,range(2*n_fixed,3*n_fixed)])

#         # rescale slopes so that slopes*sgns has the desired ell_1 norm
#         slopes = self.radius * slopes/(slopes*sgns).abs().sum(dim=-1,keepdim=True).sum(dim=-2,keepdim=True).expand_as(slopes)
#         slopes = dup_along_newdim(slopes,-3,n_obs)
#         sgns = dup_along_newdim(sgns,-3,n_obs)

#         one_d_funs = (slopes*F.relu(sgns*(w_obs[:,:,range(s)].unsqueeze(-1).expand_as(locs)-locs))).sum(dim=-2).sum(dim=-1).unsqueeze(-1)

#         # generate y
#         y = one_d_funs[:,range(n),:] + self.E.sample([nbatch,n])
        
#         return w_obs[:,range(-n_tilde,0),:], w_obs[:,range(n),:], y, one_d_funs
...
class PriorGam(nn.Module):
    ...
    def forward(self, nbatch, n, n_tilde=1):
        ...
        # nbatch x num_a x n_fixed x s output
        one_d_funs = self.rn(self.noise_sampler.sample([nbatch,self.num_a,n_fixed]))

        # Flip sign of increments at random
        one_d_funs = one_d_funs * (2*self.bern5.sample([nbatch,self.num_a,n_fixed,s]).to(device)-1)

        # define variation norm
        var_norm = torch.abs(one_d_funs).sum(dim=-1).sum(dim=-1)
        ...
        one_d_funs = torch.gather(one_d_funs.permute(0,1,3,2),3,dup_along_newdim(rank_inds_w_fixed,1,self.num_a)).permute(0,1,3,2)
        one_d_funs = torch.cat((one_d_funs,torch.zeros(nbatch,self.num_a,n_obs,s,device=device)),dim=-2)
        one_d_funs = torch.gather(one_d_funs.permute(0,1,3,2),3,dup_along_newdim(order_inds,1,self.num_a)).permute(0,1,3,2)

        # Take the cumulative sum of the increments to get the regression function
        one_d_funs = torch.cumsum(one_d_funs,dim=2)

        # Reorder the one_d_funs to correspond to the ranks of the coordinates of ws
        one_d_funs = torch.gather(one_d_funs.permute(0,1,3,2),3,dup_along_newdim(rank_inds,1,self.num_a)).permute(0,1,3,2)

        # for each observation, sum out the one_d_funs across the coordinates of w
        one_d_funs = one_d_funs.sum(dim=3).unsqueeze(-1)

        # generate y
        # CAUTION: hard-coded for the case where self.num_a=2 and a takes values 0 and 1
        y = (1-a) * one_d_funs[:,0,range(-n_obs,-n_tilde),:] + a * one_d_funs[:,1,range(-n_obs,-n_tilde),:] + self.E.sample([nbatch,n])
    
        return w_obs[:,range(-n_tilde,0),:], w_obs[:,range(-n_obs,-n_tilde),:], a, y, one_d_funs[:,1,range(-n_obs,0),:] - one_d_funs[:,0,range(-n_obs,0),:]
    
####################################################################################
# Simulation settings from the outcome weighted learning paper

# generate data according to simulation scenarios in the OWL paper, but with 10 covariates
# nbatch: number of datasets to generate
# n: sample size
# outputs are:
#  w: covariates; nbatch x n x wdim tensor
#  a: treatment; nbatch x n x 1 tensor
#  y: outcome; nbatch x n x 1 tensor
#  cate: true CATE; nbatch x n x 1 tensor

def sim_owl(n,nbatch=1,setting = 1,wdim = 10):
    w = 2*torch.rand([nbatch,n,wdim],device=device)-1
    a = bernoulli.Bernoulli(0.5).sample([nbatch,n,1]).to(device)
    if setting == 1:
        T0_init = 0.442 * (1 - w[:,:,0] - w[:,:,1])
    elif setting == 2:
        T0_init = w[:,:,1] - 0.25*torch.pow(w[:,:,0],2) - 1
    elif setting == 3:
        T0_init = (0.5-torch.pow(w[:,:,0],2) - torch.pow(w[:,:,1],2))*(torch.pow(w[:,:,0],2) + torch.pow(w[:,:,1],2) - 0.3)
    elif setting == 4:
        T0_init = 1 - torch.pow(w[:,:,0],3) + torch.exp(torch.pow(w[:,:,2],2)+w[:,:,4]) + 0.6*w[:,:,5] - torch.pow(w[:,:,6]+w[:,:,7],2)
    T0_init = T0_init.unsqueeze(-1)
    T0 = T0_init*(2*a-1)
    y = T0 + torch.randn([nbatch,n,1],device=device)
...
# Prior class that generates data from sim_owl function
# (included so that can l2p.interrogate can be used to evaluate an estimator on the OWL sim settings)

class PriorOWL(nn.Module):
    def __init__(self, wdim, setting):
        super(PriorOWL, self).__init__()
    ...
    def forward(self, nbatch, n, n_tilde=1):
        n_obs = n + n_tilde
        w, a, y, cate = sim_owl(n_obs,nbatch,setting=self.setting,wdim=self.wdim)
        
        return w[:,range(-n_tilde,0),:], w[:,range(-n_obs,-n_tilde),:], a[:,range(-n_obs,-n_tilde)], y[:,range(-n_obs,-n_tilde),:], cate

####################################################################################
# # Generic equivariant estimator that can be used in prediction problems (doesn't take advantage of linear model or GAM structure)
# # w_tilde is fed as input along with (w, y) prior to mean pooling

# class Predictor(nn.Module):
#     def __init__(self, hidden_size1, num_hidden1, pool_size1, hidden_size2, num_hidden2, pool_size2, hidden_size3, num_hidden3, pool_size3, hidden_size4, num_hidden4, output_size, rank_based = False, standardize_pred = "mean"):
...
#         if standardize_pred is "ols" or standardize_pred is "mean":
#             self.standardize_pred = standardize_pred
#         else:
#             raise Exception("Invalid standardize_pred. standardize_pred must be either ols or mean.")

#     def beta_ols(self, w, y):
#         return torch.matmul(torch.matmul(torch.inverse(torch.matmul(w.transpose(-1,-2),w)),w.transpose(-1,-2)),y)

#     # def resid_sd(self, w, y):
#     #     # w is nbatch x n x wdim  or  n x wdim
#     #     # y is nbatch x n x 1  or  n x 1
#     #     return torch.sqrt((torch.matmul(y.transpose(-1,-2),y) - torch.matmul(torch.matmul(y.transpose(-1,-2),torch.matmul(torch.matmul(w,torch.inverse(torch.matmul(w.transpose(-1,-2),w))),w.transpose(-1,-2))),y))/w.size()[-2]).squeeze(-1)
...
#         if self.standardize_pred == "mean":
#             sigma_pred = sigma
#             ctr_test = dup_along_newdim(mu,1,n_tilde).squeeze(-1)
#         elif self.standardize_pred == "ols":
#             beta = self.beta_ols(w,y)
#             sigma_pred = torch.sqrt(torch.mean((y-torch.matmul(w,beta))**2,dim=1))
#             ctr_test = torch.matmul(w_tilde,beta).squeeze(-1)
...
#         x = x * sigma_pred.expand_as(x) + ctr_test
...
# THE BELOW WAS USED WHEN TRAINING THE gams!
class Predictor(nn.Module):
    def __init__(self, hidden_size1, num_hidden1, pool_size1, hidden_size2, num_hidden2, pool_size2, hidden_size3, num_hidden3, pool_size3, hidden_size4, num_hidden4, output_size, rank_based = False):
        ...
        self.np_net = equivar_net(3, num_hidden1, hidden_size1, pool_size1, [-3,-2], activation="leaky_relu", pool="mean")
        self.p_net1 = equivar_net(pool_size1, num_hidden2, hidden_size2, pool_size2, [-2], activation="leaky_relu", pool="mean")
        self.p_net2 = equivar_net(pool_size2+1, num_hidden3, hidden_size3, pool_size3, [-2], activation="leaky_relu", pool="mean")
        self.out_net = ReLU_net(pool_size3, num_hidden4, hidden_size4, 1, leaky=True)
    ...
    def forward(self, w_tilde, w, a, y): 
        ...
        # first compute "sufficient statistics" from the data (don't use w_tilde)
        x = torch.cat((w.unsqueeze(3),dup_along_newdim(a,dim=2,num_rep=wdim),dup_along_newdim(y,dim=2,num_rep=wdim)),dim=3)
...
def initT(lr=0.001,rank_based=False,gam=False,ablation=0):
    if ablation==0:
        T = Predictor(hidden_size1 = 100,
                         ...
                         #standardize_pred = "mean" if gam else "ols").to(device)
...
def load_model(T, T_opt, T_sched, Pi, Pi_opt, Pi_sched, fl, fl_backup = None, device=device):
    """load all models + optimizers to resume training from prev checkpoint"""
...
####################################################################################
# CAUTION: need to add n and n_tilde as inputs
def interrogate(T,Pi,n,n_tilde,numbatch=100,batchsize=1000):
    ...
    for i in range(numbatch):
        ...
        with torch.no_grad():
            w_tilde, w, a, y, regfun = Pi(batchsize,n,n_tilde=n_tilde)

        # evaluate T
        T_out = T(w_tilde, w, a, y).detach()
...
def train(niter,n,n_tilde,nbatch,T,T_opt,T_sched,Pi,Pi_opt,Pi_sched,fl,loadm=False,new_T_lr=None,new_Pi_lr=None,fixedPi=False,nT=1,firstPiIter=0):
    ...
    while iteration<=niter:        
        ...
        for _ in range(nT):
            ...
            # sample from Pi
            with torch.no_grad():
                w_tilde, w, a, y, regfun = Pi(nbatch,n,n_tilde=n_tilde)
                
            # get estimate of mu(x) from procedure network
            T_out = T(w_tilde, w, a, y)
            # CAUTION: uncomment below two lines for slightly better convergence but 2x memory usage
#             T_out2 = T(w_tilde, w, a, -y)

            # get loss and gradient
#             T_loss = MSE(T_out, regfun[:,range(-n_tilde,0),:].squeeze(-1)) + MSE(-T_out2, regfun[:,range(-n_tilde,0),:].squeeze(-1))
            T_loss = MSE(T_out, regfun[:,range(-n_tilde,0),:].squeeze(-1))
        ...
        # print diagnostics
        if (iteration % 2000 == 0) or (iteration==niter):
            print('Iteration ',
